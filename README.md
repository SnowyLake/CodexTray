...
将鼠标悬停在卡片名称或状态圆点上查看错误信息. DeepSeek 和 NewAPI 需要有效的 Base URL 与凭据, NewAPI 还需要 User ID. Grok 需要先在所选的 Grok Build 或 OpenCode 中完成 xAI OAuth 登录. 日常使用中 CodexTray 会自动刷新 access token; 若 refresh token 也失效, 再回到对应工具重新登录.
